...
//https://fiddle.jshell.net/MKGaru/uwL69kwk/3/light/

(ko.bindingHandlers as any).clusterize = {
    init : (element: Element, valueAccessor, allBindings, viewModel, bindingContext) => {
        const childElem = document.createElement("div");
        element.appendChild(childElem);
        var clusterize = new Clusterize({
            scrollElem: element,
            contentElem: childElem,
            rows_in_block:50,
            blocks_in_cluster:4
        });
        
        ko.utils.domData.set(element, "clusterize", clusterize);
    },
    update:(element: Element, valueAccessor, allBindings, viewModel, bindingContext) => {
        var clusterize = ko.utils.domData.get(element, "clusterize") as any;
        
        clusterize.update($componentTemplateNodes)
